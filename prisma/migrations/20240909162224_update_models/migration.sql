/*
  Warnings:

  - You are about to drop the column `pricePerDay` on the `Car` table. All the data in the column will be lost.
  - Added the required column `priceForDay` to the `Car` table without a default value. This is not possible if the table is not empty.
  - Added the required column `title` to the `Car` table without a default value. This is not possible if the table is not empty.

*/
-- AlterTable
ALTER TABLE "Car" DROP COLUMN "pricePerDay",
ADD COLUMN     "priceForDay" DOUBLE PRECISION NOT NULL,
ADD COLUMN     "title" TEXT NOT NULL;
